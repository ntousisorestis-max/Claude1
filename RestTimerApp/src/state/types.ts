...
export type WorkoutState = {
  ...
  appsLocked: boolean;
  /**
   * Whether the welcome screen has been tapped through.
   *
   * Saved rather than session-only — it is the whole point of the screen that
   * it appears exactly once. Kept on the same payload as the exercises so
   * there is one load on launch and one thing that can be stale, rather than
   * two races against the splash.
   */
  welcomed: boolean;
  /**
...
export type WorkoutAction =
  ...
  | { type: 'WELCOME_DONE' }
  | { type: 'RENAME_EXERCISE'; id: string; name: string }
